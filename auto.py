...
###########################################################
# "랜덤결과" 리스트 (이제: 적게 배정된 사람 리스트, 우선 배정용)
###########################################################
def load_history():
    # 이제 단순 이름 리스트로 사용
...
def assign_one_period(staff_list, period, demand, is_morning):
    ...
    # 전교시 미배정 보정(지금 구조에선 사실상 영향 거의 없음, 기존 로직 그대로 유지)
    for s in staff_list:
    ...
    for type_code, need in order:
        for _ in range(need):
            ...
            # 🔥 우선 배정 로직:
            #    - candidates 중에서 history(적게 배정된 사람 리스트)에 있는 사람들을 먼저 본다
...
# Streamlit UI
###########################################################
st.title("🚗 도로주행 자동 배정 (근무자 수정 + 가중치=1 + 우선배정)")
