##############################################################
# auto.py — 도로주행 자동 배정 (최종 통합판 - 100% 매칭 + 몰아주기 + 초기화 기능)
...
with tab_m:
    ...
    with col_m2:
        # [추가됨] 오전 탭 가중치 초기화 버튼
        if st.button("🔄 가중치(랜덤기록) 초기화", key="reset_weights_m", help="저장된 랜덤 히스토리를 삭제하여 가중치를 리셋합니다."):
            save_history([])
            st.warning("가중치(랜덤 기록)가 초기화되었습니다.")
            st.rerun()
...
with tab_r:
    ...
    # [추가됨] 랜덤 탭 초기화 버튼
    if st.button("🗑 랜덤 기록 초기화", key="reset_history_r"):
        save_history([])
        st.warning("랜덤 히스토리가 초기화되었습니다.")
        st.rerun()
